...
**Sidebar (partners only):** Partner console (`/dashboard/partner`) — client portfolio with setup checklists (program published / contacts joined / owner claimed), handoff-pending badges, Open (org switch) or Request access per client, referral link, self-serve monthly earnings. The sidebar org header is a switcher dropdown for multi-org users and partners, with partner items: New client setup (creates client org, rep as owner, `referredById` stamped), Hand off to owner (one-shot `/claim/{token}` link, optional direct email to the owner), Referral link.
...
- `/admin/partners` (ADMIN_BILLING+) — monthly partner rev-share statements: per-attributed-org collected Stripe revenue (paid EUR invoices in month), refund clawbacks (refunds ISSUED in month), 30% share, −30€ × newly activated clients, netted payout. Shared math in `src/lib/partner-statement.ts` (also consumed by the partner console's self-serve view — one implementation, no disputes).
