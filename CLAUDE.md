...
- Organization-level roles = Better Auth **organization membership** roles, three tiers (Member.role strings): `owner` > `admin` > `member`. `admin` = **"Program manager"** (UI label): full program surfaces — design studio, distribution, program lifecycle (create/activate/reactivate/archive), pass status, media — but NO billing, team, org settings, audit log, GDPR export, or `deleteTemplate`. Enforced via `assertOrganizationRole(orgId, "admin"|"owner")` (hierarchy in DAL). StaffInvitation.role enum: `OWNER | ADMIN | STAFF` (STAFF maps to member.role "member").
- NEVER put org roles in User.role
- **Partner flag**: `User.isPartner` (platform-controlled, toggled only in /admin/users, ADMIN_OPS+, audit-logged). Partner memberships are exempt from plan `staffLimit` counting/gating; partners get the Partner console, org-switcher tools (New client setup / Hand off to owner / Referral link), and org-less dashboard access confined to `/dashboard/partner` by `PartnerOrglessGate`.
...
- **Live endpoints** (8 original routes + the STAFF-APP-FEATURES additions below):
  ...
  - **Added 2026-09-25 (STAFF-APP-FEATURES):**
    ...
    - `GET /api/v1/stats`: stamps today, rewards redeemed today (`Reward.redeemedAt`), new contacts in the last 7 days, pending stamp rewards. "Today" = `Organization.timezone` via `startOfDayInTimeZone` (`src/lib/org-time.ts`)
    - `GET /api/v1/rewards`: pending STAMP_CARD rewards (AVAILABLE, unexpired), soonest-expiring first. Coupon reward rows are excluded (they redeem through `/actions`)
    - `POST /api/v1/contacts`: counter signup. Find-or-create the contact (email → phone), issue the pass, and email it via `after()`. Open to all roles (same outcome as public `/join`). Plan `customerLimit` is enforced only when creating. 409 codes: `alreadyHasPass` (+`passInstanceId`), `contactLimit`, `subscriptionInactive`. Shared helpers are in `src/lib/issue-pass.ts`, also used by `issuePassToContacts`
    - `GET`/`POST /api/v1/announcements` (admin+ via `requireRole`): quota + per-program reach, and sending. The core is `sendAnnouncement` in `src/lib/announcements.ts`, shared with the dashboard action; the route passes `schedule: after`. 409 codes: `quotaReached` (+`nextAvailableAt`), `programDailyCap`, `noRecipients`, `subscriptionInactive`
    - `POST /api/v1/passes/[id]/undo-stamp`: undoes only the pass's latest interaction, only a STAMP, only within 10 min. Staff can undo their own stamps; admin/owner can undo anyone's. If the stamp completed the card, its reward is deleted too, and the undo is refused (`rewardAlreadyUsed`) once that reward was redeemed, expired, or its prize revealed. It hard-deletes the Interaction, restores the counters, recomputes `lastInteractionAt`, dispatches a wallet update, and logs `STAMP_UNDONE` to OrgAuditLog (migration `20260925120000`)
  - `ApiError` accepts an `extra` record merged into the problem body (machine-readable `code`s); `requireRole(ctx, "admin"|"owner")` is the route-side role gate
...
- **Auth rate-limit policies**: `src/lib/auth-rate-limit.ts` exports per-scope helpers backed by `@upstash/ratelimit` sliding window with an in-memory fallback. **Fails open**: if the Upstash call throws (dead/deleted DB, network), the error is logged + sent to Sentry and the check degrades to the per-instance in-memory limiter — a Redis outage must never 500 the auth endpoints (2026-08-18 incident). `checkEmailSigninLimit(email, ip)` runs both a 10/min (email+ip) and a 30/h (email) check; `checkGoogleMobileLimit(ip)`, `checkDevicePairClaimLimit(ip)`, `checkInviteValidateLimit(ip)` are per-ip 10–30/min.
...
- **Raw SQL vs mapped enums**: `$queryRaw` returns the `@map`-ped DB values (lowercase, e.g. `"available"`), NEVER Prisma enum names (`"AVAILABLE"`) — comparing a raw result against a Prisma enum silently always fails (bit reward-actions.ts, expire-rewards, and the v1 reward redeem route). Prefer a typed conditional `updateMany({ where: { id, status: ... } })` claim (count === 0 → conflict) over `SELECT ... FOR UPDATE` + raw status check — the UPDATE takes the same row lock and Prisma owns the enum translation.
- `db.ts` uses a lazy Proxy so PrismaClient is not constructed at import time. Its dev-only HMR staleness check compares against every `Prisma.ModelName` — never hard-code a model name there (the old `"apiKey" in client` check broke when ApiKey was deleted in the pivot and rebuilt the client + pg pool on EVERY db access in production until 2026-09-25)
...
- [x] **Phase MONITORING-2026-08-18** — Incident response + monitoring layer after the Upstash Redis DB was deleted on Upstash's side (free-tier inactivity auto-delete; `getaddrinfo ENOTFOUND unique-arachnid-4165.upstash.io`) and every staff-app login method 500'd. **Root cause:** all `/api/v1/auth/*` endpoints call `src/lib/auth-rate-limit.ts` before any auth logic, and the `limiter.limit()` call had no error handling — a dead Redis host crashed the whole endpoint (fail-closed in the worst way). **Fail-open fix:** `check()` in `auth-rate-limit.ts` and the limiter call in `src/server/contact-form-actions.ts` now wrap `limiter.limit()` in try/catch — on Upstash failure they `console.error`, `Sentry.captureException` (tags `auth-rate-limit`/`contact-form-rate-limit`), and degrade to the existing per-instance in-memory fallback instead of throwing. A Redis outage can no longer take auth down; the tradeoff is silent degradation to weak per-Lambda limits, which is why the monitoring below exists. **Health endpoint:** new `GET /api/health` (`src/app/api/health/route.ts`) pings Neon (`SELECT 1` via `db.$queryRaw`) and Upstash (REST `GET {url}/ping` with bearer token) in parallel, each with a 5s timeout, and returns `{status, checks: {database, redis}, timestamp}` — HTTP 200 when all ok, 503 when anything is `down`/`unconfigured`, `Cache-Control: no-store`. Error messages surface the network-level `cause` (e.g. `getaddrinfo ENOTFOUND …`) over the generic "fetch failed". Built for external uptime monitors (UptimeRobot / Better Stack at 5-min intervals). **Manual ops steps** (user-side, verify done): recreate Upstash DB in eu-central-1 + update `UPSTASH_REDIS_REST_URL/TOKEN` on Vercel and `.env.local`; Sentry alert rules (new issue → email, frequency spike); point an uptime monitor at `https://loyalshy.com/api/health`. **Convention going forward:** external-service calls fail open with a Sentry capture; critical dependencies get a check in `/api/health`. tsc clean, 139/139 vitest pass, endpoint smoke-tested locally (correctly reported `database: ok`, `redis: down`, 503 while the Upstash host was dead).

- [x] **Phase PARTNERS-2026-08-18** — Agency partner program, built end-to-end for the marketing-agency channel deal (client pays the agency a 100€ setup fee, agency remits 30€/client to us, we pay 30% of collected net subscription revenue — netted monthly). Shipped as commit 53a2afe → main (34002ee), deployed + verified in prod. **(1) Partner flag & seats:** `User.isPartner` (migration `20260818090000`) toggled only in /admin/users (`adminSetPartner`, ADMIN_OPS+, `USER_PARTNER_CHANGED` audit); partner memberships exempt from plan `staffLimit` counting + invite gating; side discovery — `checkStaffLimit` had ZERO callers, staff limits are now actually enforced in `sendStaffInvitation`. **(2) Org switcher:** sidebar header becomes a dropdown for multi-org users + partners; `switchActiveOrganization` (`src/server/org-switch-actions.ts`) mirrors `/api/v1/auth/select-org` (verify membership, update only the current session row); hard navigation after switch. **(3) Partner-led onboarding & handoff:** "New client setup" → `createClientOrg` (FREE/ACTIVE org, rep as owner, `referredById` + `settings.createdByPartner` stamped); `OrgHandoffToken` model (migration `20260818110000`) behind one-shot `/claim/{token}` links (sha256 at rest, 7-day expiry, regenerate invalidates, optional direct Resend email to the owner); claim page `(auth)/claim/[token]` supports signup+OTP / signin / one-click claim-as-current-session; `claimHandoff` is an atomic claim (updateMany guard) that makes the claimant owner, demotes ALL partner-held owner seats to "admin", lands the session in the org, and redirects `/dashboard?welcome=handoff` (billing-CTA banner in DashboardShell). **(4) Third org role "admin" / Program manager** (migration `20260818130000`, StaffRole ADMIN): activated the DAL's dormant middle tier; 26 server-action gates reclassified — admin gets program lifecycle (NOT deleteTemplate), full studio, distribution, pass status, media; owner keeps org settings/billing/team/audit/GDPR/handoff-links; three-role invite dialog + three-way role mappings across web/mobile accept paths + emails; `changeTeamMemberRole` supports all three (last-owner guard `newRole !== "owner"`); staff app `InviteRole` fixed to `OWNER|ADMIN|STAFF`. **(5) Referral attribution** (migration `20260818120000`): `User.referralCode` (lazily generated via `getMyReferralLink`) powers `/register?ref=` links; register page parks the code in localStorage `loyalshy_ref` (30-day TTL, survives OTP + Google OAuth) and consumes it at org creation; `Organization.referredById` is the CANONICAL attribution column (indexed, SET NULL, backfilled from `settings.createdByPartner`); invalid/non-partner/self codes silently ignored. **(6) Rev-share statement:** shared core `src/lib/partner-statement.ts` (`REV_SHARE_RATE=0.3`, `SETUP_FEE_SHARE_CENTS=3000`, all cents, EUR-only; collected = paid Stripe invoices dated in month per attributed customer; refund clawbacks = ONE global `stripe.refunds.list({created, expand: data.charge})` filtered to attributed customers — refunds claw back in the month ISSUED; "new activated client" = attributed org created in month with ≥1 ACTIVE template); admin page `/admin/partners` + partner self-serve view call the same builder (`getPartnerStatement` ADMIN_BILLING / `getMyPartnerStatement` always caller's own id). **(7) Cohort retention:** `/admin/cohorts` — signup-month cohorts × M0-M11, retained = ≥1 Interaction that calendar month (raw SQL distinct org+month), segments all/partner/organic, size-weighted M1/M3 comparison cards. **(8) Partner console:** `/dashboard/partner` — portfolio with checklists (programPublished / contactsJoined / ownerClaimed = non-partner owner exists), handoff-pending badges, Open (member) / Request access (non-member) per client, referral card, monthly earnings. **(9) UX fixes:** org-less partners fall through the dashboard layout (no register wall) confined to the console by `PartnerOrglessGate`; handoff dialog "Generate & send" emails the owner directly (link still returned if send fails); `requestClientAccess` (partner-only, own-attribution-only, 3/h/org rate limit, `ACCESS_REQUESTED` audit, migration `20260818140000`) emails owners a deep link `/dashboard/settings?tab=team&invite={email}&inviteRole=admin` which prefills + auto-opens the Team invite dialog. **CRITICAL DISCOVERY:** the proxy's `x-pathname` header NEVER reaches `headers()` in streamed layout renders — pathname-dependent layout redirects moved to client gates (`AuthRedirectGate`, `PartnerOrglessGate`); the (auth) layout's /invite//claim signed-in exemption only actually works since this fix. **Testing:** 176/176 vitest (37 new tests across handoff/referral/statement/cohorts/console/access-request), Playwright E2E scripts in `scripts/e2e-*.ts` with fixture seed (`scripts/seed-handoff-e2e.ts`) + defensive teardown (`scripts/teardown-e2e-fixtures.ts`, `--dry-run` support); E2E requires `next dev` on port 3000 (Better Auth 403s other origins). Dev-DB migration state was repaired (`post_pivot_drift` baselined via `migrate resolve --applied`; `migrate deploy` now works locally). i18n en/es/fr for every new surface. **Ops reminder:** verify the first real month's statement against the Stripe dashboard by hand — the Stripe leg is unit-tested but has never touched real invoices. **Amendment (same day):** the partner console's self-serve earnings section and `getMyPartnerStatement` were REMOVED — rev-share rates are negotiated per partner, not a fixed 30%, so the statement is an admin-only internal tool; if terms diverge across partners, add per-partner rate columns (e.g. `User.revSharePct` / `setupFeeShareCents`) and thread them through `buildPartnerStatement`.

- [x] **Phase ANNOUNCEMENTS-2026-08-18** — Per-program wallet broadcast ("2x1 today" → lock-screen notification on every Apple/Google Wallet pass of a template). **Storage:** new `PassTemplate.announcement Json?` column (migration `20260818150000_template_announcement`) holding `{message, sentAt, history: [ISO timestamps]}` — deliberately NOT inside `config` because `validateTemplateConfig` strips unknown keys on program edits. Parser + quota helpers in `src/lib/pass-config.ts` (`parseTemplateAnnouncement`, `recentAnnouncementSends`, `ANNOUNCEMENT_MAX_PER_24H=3` — mirrors Google's ~3 TEXT_AND_NOTIFY/object/24h cap). **Apple mechanism:** announcement back field (`key: "announcement"`, `changeMessage: "%@"`) is ALWAYS present on generated passes with a "No announcements yet" placeholder — iOS only fires the banner for a *changed* field value, never a newly-added field, so shipping the placeholder makes the first real broadcast bannerable. Broadcast = store message → `update-all-passes` fan-out → `notifyApplePassUpdate` touches `updatedAt` (defeats the fetch route's If-Modified-Since 304) + APNs push → device refetch → banner shows the raw message. **Google mechanism:** ONE class-level PATCH (`src/lib/wallet/google/announce.ts`, `sendGoogleClassAnnouncement`) with a `TEXT_AND_NOTIFY` message id'd `announce-{sentAt ms}` notifies every holder of the template's loyalty class in a single call (404 = class never created = no-op). `buildLoyaltyClass` re-includes the current announcement with the SAME deterministic id so the issuance path's best-effort class PATCH can't wipe the broadcast, and Google's id-dedupe never re-notifies. **Server action:** `src/server/announcement-actions.ts` `sendProgramAnnouncement` — admin+ role gate, Zod 1–160 chars, ACTIVE templates only, 3/24h rolling quota from `history`, Google class PATCH best-effort (failure never fails the send), Apple/Google object fan-out via Trigger.dev `update-all-passes` (reason TEMPLATE_CHANGE) with the standard direct-notify fallback when `TRIGGER_SECRET_KEY` is empty; returns `recipients` (ACTIVE instances with a wallet provider). **Plumbing:** `announcement` selected + passed at all 7 generator call sites (apple v1 fetch route, legacy apple route, download route, google save-url route, wallet-actions, onboarding-actions `issuePassForInstance`, generate-pass-for-email). **UI:** `AnnouncementSection` card on `/dashboard/programs/[id]/distribution` (Megaphone icon, 160-char textarea + counter, remaining-quota badge, last-sent preview, AlertDialog confirm with holder count via ICU plurals). i18n: 12 keys × 3 locales in `dashboard.distribution` + 2 `serverErrors` keys. **Tests:** 9 vitest cases in `announcement-actions.test.ts` (validation, org scoping, non-active reject, quota enforce/expiry/append, Google-failure tolerance) — 200/200 pass, tsc clean, `next build` clean. **Caveats:** delivery is best-effort (users can mute per-pass notifications; pre-apex-fix Apple passes with `www.` webServiceURL still can't update); the first broadcast to passes issued BEFORE this phase updates silently on Apple (field appears without banner) — subsequent ones banner normally.
...
8. PassTemplate (passType: `STAMP_CARD | COUPON`, status: DRAFT/ACTIVE/ARCHIVED, config JSON, startsAt, endsAt — no `joinMode`, all programs are open self-join; `announcement Json?` holds the wallet broadcast `{message, sentAt, history}` — own column because `validateTemplateConfig` strips unknown config keys)
...
20. OrgAuditLog (org-scoped trail of owner actions — invitation sent/resent/cancelled/accepted, member removed, member role changed, handoff link created, ownership claimed, access requested; nullable actor with denormalized actorEmail so the row survives if the actor user is deleted)
21. OrgHandoffToken (partner-led onboarding: one-shot ownership-claim link per client org; sha256 token at rest, 7-day expiry, regenerate invalidates; claim atomically makes the claimant owner and demotes partner-held owner seats to "admin")

**Partner-program columns:** `User.isPartner` (admin-controlled flag), `User.referralCode` (unique, lazily generated, powers `/register?ref=` links), `Organization.referredById` (canonical partner attribution — indexed FK SET NULL, set by partner org creation AND referred self-signup; drives the rev-share statement `src/lib/partner-statement.ts`: 30% of collected net EUR revenue − 30€ × newly activated clients, netted).
...
**Sidebar (partners only):** Partner console (`/dashboard/partner`) — client portfolio with setup checklists (program published / contacts joined / owner claimed), handoff-pending badges, Open (org switch) or Request access per client, referral link. **No earnings view** — rev-share rates are negotiated per partner (not the fixed 30%), so statements are admin-only (`/admin/partners`); removed 2026-08-18. The sidebar org header is a switcher dropdown for multi-org users and partners, with partner items: New client setup (creates client org, rep as owner, `referredById` stamped), Hand off to owner (one-shot `/claim/{token}` link, optional direct email to the owner), Referral link.
...
- `/dashboard/programs/[id]/distribution` — Distribution: QR/NFC self-service link, direct issue to contacts, send announcement (wallet broadcast to all pass holders, plan-based per-org quota + 3/24h per-program cap) (owner only). No join-mode toggle — every program is open self-join.
...
- `/admin/partners` (ADMIN_BILLING+) — monthly partner rev-share statements: per-attributed-org collected Stripe revenue (paid EUR invoices in month), refund clawbacks (refunds ISSUED in month), 30% share, −30€ × newly activated clients, netted payout. Math in `src/lib/partner-statement.ts` with fixed constants (`REV_SHARE_RATE=0.3`, `SETUP_FEE_SHARE_CENTS=3000`) — **admin-only, internal payout tool**: actual rates are negotiated per partner, so partners never see this (the console's self-serve earnings view was removed 2026-08-18; per-partner rate columns are a future change if terms diverge).
- `/admin/cohorts` (ADMIN_SUPPORT+) — cohort-retention grid: signup-month cohorts × M0-M11, retained = ≥1 Interaction that calendar month, segmented all/partner/organic by `referredById`; weighted M1/M3 comparison cards.
