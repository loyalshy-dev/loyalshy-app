...
- **Token storage**: invitation + pairing tokens are stored as `sha256(plaintext)` hex via `src/lib/token-hash.ts`. Plaintext only ever leaves in the QR payload, deep link, or email body. Resend rotates the plaintext.
- **Wallet update dispatch**: shared helper at `src/lib/wallet/dispatch.ts` consumed by `passes/[id]/actions` and `rewards/[id]/redeem`.
- **Key files**: `src/lib/api-session.ts` (`sessionHandler` + `sessionHandlerNoOrg` + `ApiError`), `src/lib/auth-rate-limit.ts`, `src/lib/token-hash.ts`, `src/lib/api-serializers.ts` (toApiContact, toApiPassInstance, toApiPassInstanceDetail, toApiReward, toApiInteraction, toApiTemplate), `src/lib/api-cors.ts`, `src/lib/wallet/dispatch.ts`. Action transactions are inlined in the route files (not in shared `api-data.ts` — the old shared layer is gone).
